...
const BrandingPage: React.FC = () => {
    ...
    return (
        <section ref={containerRef} className="relative min-h-screen w-full overflow-hidden bg-[#EBE9DF] text-[#1A1A1A]">
            ...
            <div ref={philosophyRef} className="h-screen w-full bg-[#1A1A1A] text-[#EBE9DF] flex flex-col items-center justify-center relative overflow-hidden">
                ...
                >
                    BLAND IS A <span className="text-[#D16D6A]">DEATH SENTENCE.</span>
                </h2>
                ...
                <div ref={philosophyContentRef} className="max-w-[90rem] mx-auto px-6 md:px-12 grid md:grid-cols-2 gap-12 relative z-20 opacity-100">
                    <p className="font-mono text-xl md:text-2xl text-white/80 leading-relaxed bg-[#1A1A1A]/50 p-6 backdrop-blur-sm border-l-4 border-[#D16D6A]">
                    ...
                    <p className="font-mono text-xl md:text-2xl text-white/80 leading-relaxed bg-[#1A1A1A]/50 p-6 backdrop-blur-sm">
            ...
            <div className="py-32 px-6 md:px-12 max-w-[90rem] mx-auto">
                <ScrollReveal staggerIndex={0}>
                    <div className="mb-24">
                        <h2 className="text-sm font-mono font-bold tracking-[0.3em] uppercase text-[#D16D6A] mb-4">Core Deliverables</h2>
                ...
                <div className="space-y-32">
                    {capabilities.map((cap, index) => (
                        <div key={cap.id} className={`grid lg:grid-cols-12 gap-12 items-center ${index % 2 !== 0 ? 'lg:grid-flow-col-dense' : ''}`}>
                            ...
                            <div className={`lg:col-span-5 h-[400px] bg-[#1A1A1A] relative overflow-hidden flex items-center justify-center p-8 group ${index % 2 !== 0 ? 'lg:col-start-8' : ''}`}>
                                <ScrollReveal staggerIndex={1}>
                                    ...
                                    <div className="absolute w-24 h-24 border border-[#D16D6A] rounded-full animate-spin-slow opacity-50"></div>
                                    <div className="absolute w-2 h-2 bg-[#D16D6A] rounded-full animate-ping"></div>
                                </ScrollReveal>
                            </div>
                            ...
                            <div className={`lg:col-span-7 flex flex-col ${index % 2 !== 0 ? 'lg:col-start-1 lg:col-end-8' : ''}`}>
                                <ScrollReveal staggerIndex={2}>
                                    ...
                                    <div className="mb-10">
                                        <h5 className="font-bold text-xs font-mono uppercase tracking-widest text-[#1A1A1A] mb-4 border-b border-[#1A1A1A]/10 pb-2">Included Artifacts:</h5>
                                        <ul className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                            {cap.deliverables.map((item, i) => (
                                                <li key={i} className="flex items-start">
                                                    <div className="w-1.5 h-1.5 bg-[#D16D6A] mt-2 mr-3 rounded-full flex-shrink-0"></div>
                                    ...
                                    <button 
                                        ...
                                        className="inline-flex items-center justify-center px-8 py-4 bg-[#1A1A1A] text-white font-mono text-xs font-bold uppercase tracking-widest hover:bg-[#D16D6A] transition-colors w-max"
                                    >
                                        Initiate Protocol
                                    </button>
                                </ScrollReveal>
            ...
            {/* METHODOLOGY SECTION */}
            <div className="bg-[#EBE9DF] border-t-2 border-[#1A1A1A] py-32 px-6 md:px-12 overflow-hidden">
                <div className="max-w-[90rem] mx-auto">
                    ...
                    <div className="grid md:grid-cols-3 gap-8 md:gap-12 relative">
                        <div className="hidden md:block absolute top-[40px] left-0 w-full h-[2px] bg-[#1A1A1A]/10 -z-10"></div>
                        {processes.map((step, index) => (
                            <ScrollReveal key={step.num} staggerIndex={index + 1}>
                                <div className="bg-white p-8 md:p-10 border border-[#1A1A1A]/10 hover:border-[#D16D6A] transition-colors h-full flex flex-col">
                                    <div className="w-16 h-16 bg-[#1A1A1A] text-[#EBE9DF] font-bold font-mono flex items-center justify-center text-xl mb-8 group-hover:scale-110 transition-transform">
                                        {step.num}
                                    </div>
                                    <h4 className="text-2xl font-black uppercase mb-4">{step.title}</h4>
                                    <p className="font-mono text-sm text-[#1A1A1A]/70 leading-relaxed flex-grow">
                                        {step.desc}
                                    </p>
                                </div>
                            </ScrollReveal>
                ...
                <div ref={ctaRef} className="relative p-12 md:p-20 bg-gradient-to-br from-[#D16D6A] to-[#B04E4B] overflow-hidden">
                    ...
                    <div className="relative z-10 max-w-2xl">
                        ...
                        <a href="mailto:hello@arsonpixels.com?subject=Brand%20Ignition%20Inquiry"
                           className="inline-block px-10 py-5 bg-[#1A1A1A] text-white font-mono font-bold uppercase tracking-widest hover:bg-white hover:text-[#1A1A1A] transition-colors"
                        >
                            Open Comms
                        </a>
                    </div>
