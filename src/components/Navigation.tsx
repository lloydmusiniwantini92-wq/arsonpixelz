...
// --- Integrated Shop Badge ---
// Removed Standalone CartTrigger as it's now part of the Shop Nav link
...
const TopNavLink = memo(({ 
    ...
    return (
        <Link
            to={href}
            className={`relative px-4 py-2.5 group flex flex-col items-center justify-center gap-1.5 transition-all duration-500 ${
                isHUDActive ? 'px-6' : 'px-4'
            }`}
        ...
        >
            {/* HUD Bracket Frames */}
            {isHUDActive && (
                <div className="absolute inset-x-0 inset-y-1 pointer-events-none transition-opacity duration-500" style={{ opacity: hudIntensity }}>
                    <div className="absolute left-0 top-0 w-1.5 h-1.5 border-l border-t border-[#D16D6A]/60" />
                    <div className="absolute right-0 top-0 w-1.5 h-1.5 border-r border-t border-[#D16D6A]/60" />
                    <div className="absolute left-0 bottom-0 w-1.5 h-1.5 border-l border-b border-[#D16D6A]/60" />
                    <div className="absolute right-0 bottom-0 w-1.5 h-1.5 border-r border-b border-[#D16D6A]/60" />
                    
                    {/* Data Pip */}
                    <div className="absolute -top-1 left-1/2 -translate-x-1/2 w-[2px] h-[2px] bg-[#D16D6A] group-hover:scale-150 transition-transform" />
                </div>
            )}

            <div className={`relative z-10 font-mono text-[10px] md:text-xs font-bold uppercase tracking-[0.2em] transition-all duration-500 will-change-transform ${isActive || hasItems
                ? 'text-[#D16D6A]'
                : onLightBackground
                    ? 'text-[#0F0F0F]/70 group-hover:text-[#D16D6A]'
                    : isHUDActive ? 'text-[#EBE9DF]/80 group-hover:text-[#D16D6A]' : 'text-[#EBE9DF]/70 group-hover:text-[#D16D6A]'
                } ${isRevealed ? '-translate-y-1' : 'translate-y-4'}`}
                style={{
                    textShadow: (isHUDActive && isActive) || hasItems ? '0 0 10px rgba(209,109,106,0.6)' : 'none'
                }}
            >
                <ScrambleText text={label} active={hovered} />
            </div>

            <div className={`relative z-10 transition-all duration-500 will-change-transform ${
                hasItems ? 'animate-[cart-pulse_2s_ease-in-out_infinite]' : ''
            } ${isRevealed ? 'opacity-100 translate-y-0 scale-100' : 'opacity-0 translate-y-4 scale-75'}`}>
                <Icon className={`w-7 h-7 transition-all duration-500 ${hovered || isActive || hasItems
                    ? 'text-[#D16D6A]'
                    : onLightBackground
                        ? 'text-[#0F0F0F]/40'
                        : isHUDActive ? 'text-[#EBE9DF]/40' : 'text-[#EBE9DF]/70'
                    }`} 
                    style={{
                      filter: (isHUDActive || hasItems) && (hovered || isActive || hasItems) ? 'drop-shadow(0 0 8px rgba(209,109,106,0.4))' : 'none'
                    } as React.CSSProperties}
                />

                {/* Integrated Cart Badge */}
                {hasItems && (
                     <span className="absolute -top-1 -right-1 w-4 h-4 bg-[#D16D6A] text-white text-[9px] font-black flex items-center justify-center rounded-full animate-bounce shadow-[0_0_10px_rgba(209,109,106,0.5)]">
                        {cartCount}
                    </span>
                )}
            </div>

            <div className={`absolute inset-0 ${onLightBackground ? 'bg-black/5' : isHUDActive ? 'bg-[#D16D6A]/5' : 'bg-white/5'} rounded-sm transform origin-bottom transition-transform duration-300 ${'scale-y-0 group-hover:scale-y-100'
                }`} />
        </Link>
    );
});
...
// --- QUANTUM LOGO WITH SIZZLING FIRE REVEAL ---
    ...
    return (
        <div className="group relative flex items-start gap-1 select-none pr-1">
            ...
            <div className={`relative overflow-visible transition-all duration-700 ease-[cubic-bezier(0.22,1,0.36,1)] origin-left
                ...
                style={{
                  filter: isHUDActive ? `drop-shadow(0 0 15px rgba(209,109,106,${0.3 * hudIntensity}))` : 'none'
                }}
            >
                {/* LAYER 1: Base Invisible Paths */}
                ...
                {/* LAYER 2: Solid Core Logo */}
                <div 
                    className="absolute inset-0 w-full h-full" 
                    style={{ 
                        animation: isHUDActive ? 'none' : 'core-sweep 5.5s cubic-bezier(0.16,1,0.3,1) both',
                        animationDelay: '3.5s' 
                    }}
                >
...
// --- MENU LINK COMPONENT ---
const MenuLink = memo(({ link, href, index, onClick, tag, isHovered, onHoverChange }: {
    link: string;
    href: string;
    index: number;
    onClick: () => void;
    ...
    return (
        <Link
            ...
            className="group flex items-baseline gap-4 py-3 md:py-4 border-b border-white/5 hover:border-[#D16D6A]/40 transition-all duration-300 px-8 md:px-12"
            style={{
                transform: isHovered ? 'scale(1.02) translateX(8px)' : 'scale(1) translateX(0)',
                transition: 'transform 0.35s cubic-bezier(0.16,1,0.3,1), border-color 0.3s',
            }}
        >
            <span
                className="font-mono text-[10px] tracking-[0.3em] w-8 shrink-0 transition-colors duration-300"
                style={{ color: isHovered ? '#D16D6A' : 'rgba(255,255,255,0.4)' }}
            >
                {String(index + 1).padStart(2, '0')}
            </span>
            <span
                className="flex-1 font-black uppercase tracking-tighter leading-none transition-all duration-300"
                style={{
                    fontSize: isHovered ? 'clamp(2rem, 5vw, 4rem)' : 'clamp(1.6rem, 3.5vw, 3rem)',
                    color: isHovered ? '#D16D6A' : 'rgba(235,233,223,0.85)',
                    textShadow: isHovered ? '0 0 60px rgba(209,109,106,0.55)' : 'none',
                    fontFamily: 'Syne, sans-serif',
                    transition: 'font-size 0.35s cubic-bezier(0.16,1,0.3,1), color 0.3s, text-shadow 0.3s',
                }}
            >
                {link}
            </span>
            {tag && (
                <span className="font-mono text-[10px] tracking-widest uppercase transition-colors duration-300"
                    style={{ color: isHovered ? 'rgba(209,109,106,0.6)' : 'rgba(255,255,255,0.15)' }}>
                    {tag}
                </span>
            )}
            <span
                ...
                style={{
                    color: isHovered ? '#D16D6A' : 'rgba(255,255,255,0.15)',
...
export const Navigation: React.FC = () => {
    ...
    // Persist HUD state to detect transitions
    const { setIsCartOpen, cartCount } = useCart();
    const lastHUDState = React.useRef(false);
    const scrollTimeout = React.useRef<NodeJS.Timeout | null>(null);
    const surgeTimeout = React.useRef<NodeJS.Timeout | null>(null);

    // Listen to the global void progress
    useEffect(() => {
        const interval = setInterval(() => {
    ...
    // Detect HUD Activation Surge
    useEffect(() => {
        if (isHUDActive && !lastHUDState.current) {
            setSurgeActive(true);
            if (surgeTimeout.current) clearTimeout(surgeTimeout.current);
            surgeTimeout.current = setTimeout(() => setSurgeActive(false), 5000);
        }
        lastHUDState.current = isHUDActive;
    }, [isHUDActive]);

    const onLightBackground = useMemo(() =>
        // Force dark theme (light text) at the top of the page for Hero background legibility
        scrolled && LIGHT_BACKGROUND_ROUTES.includes(location.pathname) && voidProgress < 0.3,
        ...
        <>
            <motion.header
                ...
                className={`fixed top-0 left-0 w-full z-50 transition-all duration-700 ease-[cubic-bezier(0.2,1,0.2,1)] ${
                    scrolled
                        ? isHUDActive
                            ? "bg-[#080808]/40 py-2 md:py-3"
                            : onLightBackground
                                ? "bg-[#EBE9DF]/30 py-3 md:py-4"
                                : "bg-[#0F0F0F]/40 py-3 md:py-4"
                        : "bg-transparent py-6 md:py-8"
                }`}
                style={{
                  backdropFilter: 'blur(32px)',
                  WebkitBackdropFilter: 'blur(32px)',
                  borderBottom: isHUDActive 
                    ? `1px solid rgba(209,109,106, ${0.2 * hudIntensity})` 
                    : scrolled ? '1px solid rgba(0,0,0,0.05)' : 'none',
                  boxShadow: isHUDActive 
                    ? `0 10px 40px -10px rgba(0,0,0,0.5), 0 0 20px rgba(209,109,106,${0.05 * hudIntensity})` 
                    : 'none'
                }}
            >
                {/* ─── Tony Stark HUD Elements ─── */}
                {isHUDActive && (
                  <div className="absolute inset-0 pointer-events-none overflow-hidden" style={{ opacity: hudIntensity }}>
                    {/* Lateral Scan Tracks */}
                    <div className="absolute left-10 right-10 top-0 h-px bg-gradient-to-r from-transparent via-[#D16D6A]/30 to-transparent" />
                    <div className="absolute left-10 right-10 bottom-0 h-px bg-gradient-to-r from-transparent via-[#D16D6A]/30 to-transparent" />
                    
                    {/* Vertical Pips */}
                    <div className="absolute left-4 top-1/2 -translate-y-1/2 w-px h-8 bg-[#D16D6A]/40" />
                    <div className="absolute right-4 top-1/2 -translate-y-1/2 w-px h-8 bg-[#D16D6A]/40" />

                    {/* Scanning Line */}
                    <div className="absolute inset-0 bg-gradient-to-b from-[#D16D6A]/5 via-transparent to-transparent h-[200%] animate-[hud-scan_4s_linear_infinite]" />
                  </div>
                )}
                ...
                <style>{`
                    ...
                    @keyframes fire-rain { 
                        ...
                        100% { transform: translateY(12px) scaleY(2); opacity: 0; }
                    }
                ...
                {/* ─── Fiery Sizzle Base Decor ─── */}
                {scrolled && (
                    <div className="absolute bottom-0 left-0 w-full h-[6px] overflow-visible pointer-events-none">
                        {/* Standard Substrate */}
                        <div className={`absolute inset-0 h-[1px] ${onLightBackground ? 'bg-black/5' : 'bg-white/5'}`} />
                        
                        {/* Fiery Sizzle Surge */}
                        {isHUDActive && isScrolling && surgeActive && (
                            <div 
                                className="absolute inset-0 h-full transition-opacity duration-1000"
                                style={{ opacity: hudIntensity }}
                            >
                                {/* Sizzling Line Filtered */}
                                <svg width="100%" height="100%" className="absolute inset-0 overflow-visible">
                                    <defs>
                                        <filter id="hudSizzle">
                                            <feTurbulence type="fractalNoise" baseFrequency="0.6 0.2" numOctaves="3" result="noise" />
                                            <feDisplacementMap in="SourceGraphic" in2="noise" scale="8" />
                                        </filter>
                                    </defs>
                                    <line 
                                        x1="0" y1="3" x2="100%" y2="3" 
                                        stroke="#D16D6A" strokeWidth="2" 
                                        filter="url(#hudSizzle)" 
                                        strokeDasharray="4 2"
                                        style={{ animation: 'sizzle-wash 0.5s linear infinite' }}
                                    />
                                </svg>
                                
                                {/* Shimmer Overlay */}
                                <div className="absolute inset-0 bg-gradient-to-r from-transparent via-[#D16D6A]/40 to-transparent w-1/3 animate-[shimmer-move_2s_linear_infinite]" />

                                {/* Rain of Fire Particles */}
                                <div className="absolute inset-x-0 top-full h-8 flex justify-around pointer-events-none opacity-60">
                                    {[...Array(24)].map((_, i) => (
                                        <div 
                                            key={i} 
                                            className="w-[1.5px] h-3 bg-gradient-to-b from-[#D16D6A] to-transparent rounded-full"
                                            style={{
                                                animation: `fire-rain ${0.4 + Math.random() * 0.6}s linear infinite`,
                        ...
                        {/* Quantum Sweep (Fallback when not sizzling) */}
                        {(!isHUDActive || !isScrolling || !surgeActive) && (
                ...
                <div className="max-w-[1920px] mx-auto px-6 md:px-12 flex items-center justify-between relative z-10">
                    <motion.div
                        ...
                        transition={{ duration: 1.2, ease: [0.16, 1, 0.3, 1], delay: 3.5 }}
                    >
                    ...
                    <nav className="hidden md:flex items-center gap-2 lg:gap-5 absolute left-[43%] -translate-x-1/2" aria-label="Main navigation">
                        {navItems.map((item, i) => (
                            <motion.div
                                key={item.label}
                                ...
                                transition={{ 
                                    duration: 1, 
                                    ease: [0.16, 1, 0.3, 1], 
                                    delay: 3.6 + (i * 0.1) // Delays sequentially once header drops into viewport
                                }}
                            >
                                <TopNavLink 
                                    {...item} 
                    ...
                    <motion.div 
                        className={`flex items-center ${isHUDActive ? 'gap-4 md:gap-8' : 'gap-3 md:gap-5'} transition-all duration-700`}
                        initial={{ opacity: 0, x: 60, filter: 'blur(10px)' }}
                        animate={{ 
                            opacity: 1, 
                            x: isHUDActive ? 45 : 0, 
                    ...
                    >
                        {/* Redundant CartTrigger removed, now integrated into Shop Nav Link */}

                        <div className={`hidden md:block w-[1px] h-6 ${onLightBackground ? 'bg-black/20' : 'bg-white/20'} opacity-100`} aria-hidden="true" />
            ...
            <AnimatePresence>
                {isOpen && (
                    <motion.div
                        ...
                        className="fixed inset-0 z-[60] flex flex-col bg-[#0a0a0a] text-[#EBE9DF]"
                    ...
                    >
                        {/* ─── CONTEXTUAL IMAGE BACKGROUND ───
                             Crossfades between section images on hover                    */}
                        <div className="absolute inset-0 z-0 overflow-hidden">
                            {/* Default dark background */}
                            <div className="absolute inset-0 bg-[#050505]" />

                            {/* Crossfading image per hovered nav item */}
                                ...
                                {hoveredMenuIndex !== null && (
                                    ...
                                    >
                                        <img
                                            ...
                                            style={{ filter: 'grayscale(0.15) brightness(0.45) saturate(1.6)' }}
                                        />
                                        {/* Red tint wash on active */}
                                        <div className="absolute inset-0 bg-[#D16D6A]/10 mix-blend-screen" />
                                    </motion.div>
                                )}
                            </AnimatePresence>

                            {/* Permanent overlays — keep text legible */}
                        ...
                        {/* Red vertical stripe */}
                        <div className="absolute left-0 top-0 w-1 h-full bg-[#D16D6A] opacity-80 z-10" />

                        {/* Top bar */}
                        <div className="relative z-50 flex justify-between items-center px-8 md:px-12 py-6 border-b border-white/5">
                            <div className="flex items-center gap-4">
                                <div className="w-2 h-2 bg-[#D16D6A] rounded-full animate-pulse" />
                                <span className="font-mono text-[10px] tracking-[0.3em] text-white/40">arsonPixelz</span>
                            </div>
                            <button
                                onClick={closeMenu}
                                className="group relative w-12 h-12 flex items-center justify-center rounded-full border border-white/10 hover:bg-[#D16D6A] hover:border-[#D16D6A] transition-all duration-300 focus:outline-none"
                                aria-label="Close menu"
                            >
                                <XMarkIcon className="w-5 h-5 transition-transform duration-500 group-hover:rotate-90" />
                            </button>
                        </div>

                        {/* Nav links — staggered reveal */}
                        <nav
                            className="relative flex-1 min-h-0 flex flex-col justify-start z-50 overflow-y-auto pt-4 pb-6"
                        ...
                        >
                            {menuLinks.map((item, idx) => (
                                <motion.div
                                    ...
                                    transition={{ duration: 0.4, ease: [0.16, 1, 0.3, 1], delay: 0.05 + idx * 0.06 }}
                                >
                                    <MenuLink
                                        link={item.label}
                                        href={item.href}
                        ...
                        {/* Footer bar */}
                        <motion.div
                            className="relative z-50 flex flex-wrap justify-between items-center px-8 md:px-12 py-6 border-t border-white/5 gap-4"
                            initial={{ opacity: 0, y: 20 }}
                            animate={{ opacity: 1, y: 0 }}
                            transition={{ duration: 0.5, delay: 0.6 }}
                        >
                            <div className="font-mono text-[10px] leading-relaxed text-white/30 uppercase tracking-widest">
                                Arson Pixels ® &nbsp;|&nbsp; Est. 2024 &nbsp;|&nbsp; Sector 01
                                <Link to="/legal" className="ml-4 hover:text-white transition-colors underline underline-offset-2" onClick={closeMenu}>Legal</Link>
                            </div>
                            <div className="flex gap-6">
                                {socialLinks.map((social) => (
                                    <a
                                        key={social.name}
                                        href={social.url}
                                        className="font-mono text-[10px] font-bold uppercase tracking-widest text-white/30 hover:text-[#D16D6A] transition-colors"
                                    >
                                        {social.name}
                                    </a>
                                ))}
