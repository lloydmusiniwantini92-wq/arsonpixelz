    ...
    return (
        ...
        >
            ...
            <div className="max-w-[90rem] mx-auto grid lg:grid-cols-12 gap-16 lg:gap-24 items-center relative z-10">
                ...
                {/* --- LEFT: EDITORIAL CONTENT ENGINE (UPDATED TEXT & ANIMATION) --- */}
                <div className={`lg:col-span-7 flex flex-col space-y-10 transition-all duration-1000 ease-[cubic-bezier(0.25,1,0.5,1)] ${isVisible ? 'opacity-100 translate-y-0' : 'opacity-0 translate-y-12'}`}>
                    ...
                    <h2 className="relative text-5xl md:text-7xl lg:text-[7rem] font-black text-[#1A1A1A] leading-[0.85] tracking-tighter">
                        <span className="block opacity-60 font-mono text-2xl md:text-3xl mb-4 tracking-normal font-normal">WE ARE THE</span>
                        <span className="relative inline-block text-[#D16D6A]">
                            <span className="relative z-10">SPARK</span>

                            {/* Glitch Layers (Adapted for Light Mode using Mix-Blend-Multiply) */}
                            <span className="absolute top-0 left-0 -ml-[3px] text-[#D16D6A] opacity-70 animate-glitch-1 mix-blend-multiply" aria-hidden="true">SPARK</span>
                            <span className="absolute top-0 left-0 ml-[3px] text-cyan-600 opacity-70 animate-glitch-2 mix-blend-multiply" aria-hidden="true">SPARK</span>
                        </span>
                    </h2>

                    {/* Narrative Block (Updated Copy) */}
                    <div className="relative pl-6 md:pl-8 border-l-2 border-[#D16D6A] space-y-6 max-w-2xl">
                        ...
                        <p className="text-lg md:text-xl font-mono text-[#1A1A1A]/70 leading-relaxed">
                            In a digital landscape crowded with static noise, <span className="text-[#1A1A1A] font-bold bg-[#D16D6A]/10 px-1">Arson Pixels</span> is the aberration. We are a reactor core for brands ready to go critical.
                        </p>
                ...
                {/* --- RIGHT: 3D ARTIFACT CASE (UNCHANGED) --- */}
                <div className="lg:col-span-5 relative h-[600px] flex items-center justify-center perspective-[1000px]">
                    ...
                    >
                        ...
                        {/* 2. Main Card Body (Dark Tech on Light Bg) */}
                        <div className="absolute inset-0 bg-[#151515] rounded-sm overflow-hidden shadow-2xl border border-[#333]">
                            ...
                            >
                                <svg viewBox="0 0 200 200" className="w-[60%] h-[60%] text-[#EBE9DF]">
                                    ...
                                    <path
                                        fill="#151515"
                                        d="M 100 65 L 82 105 L 118 105 Z"
                                    />
                                </svg>
                            </div>

                            {/* 4. Glass Overlay (Reflection) */}
                            <div className="absolute inset-0 bg-gradient-to-tr from-white/5 to-transparent pointer-events-none"></div>

                            {/* 5. HUD Details on Card */}
                            <div className="absolute bottom-6 left-6 right-6 flex justify-between items-end">
                                <div className="flex flex-col">
                                    <span className="font-mono text-[10px] text-[#D16D6A] tracking-widest uppercase mb-1">Status</span>
                                    <span className="font-bold text-white text-xl tracking-tighter">UNLEASHED</span>
                                </div>
